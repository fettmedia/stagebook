...
{
	private static Logger log = Logger.getLogger(VaadinSessionManager.class);

	/**
	 * The session attribute name prefix used for storing the Shiro Session in
	 * the VaadinSession.
	 */
	private final static String	SESSION_ATTRIBUTE_PREFIX = VaadinSessionManager.class.getName()
																			+ ".session.";

	/**
	 * The session factory used to create new sessions. In the future, it may
	 * make more sense to simply implement a {@link Session} that is a
	 * lightweight wrapper on the {@link VaadinSession} rather than storing a
	...
	{
		VaadinSession result = null;
//		UI ui = UI.getCurrent();
//		if (ui != null)
//			result = ui.getSession();
		result = VaadinSession.getCurrent();
		if (result == null)
		{
			//FIXME: Handle this case, which to be honest should never happen...
		}
		return result;
	}
	...
	public Session start(SessionContext context)
	{
		...
		// Put the security session in the VaadinSession. We use the session's
		// ID as
		// part of the key just to be safe so we can double check that the
		// security
	...
	public Session getSession(SessionKey key) throws SessionException
	{
		...
		if (vaadinSession != null)
		{
			...
			if (shiroSession != null)
			{
				...
				// Make sure the Shiro Session hasn't been stopped or expired
				// (i.e. the
				// user logged out).
				...
				{
					// This is an invalid or expired session so we'll clean it
					// up.
